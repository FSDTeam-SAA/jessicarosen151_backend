...
export const addToCartService = async (userId, resourceId, quantity = 1) => {
  ...
  const effectivePrice =
    resource.discountPrice && resource.discountPrice < resource.price
  ...
  if (!cart) {
    ...
    if (existingItem) {
      ...
      if (newQuantity > resource.quantity) {
        throw new Error(
          `Total quantity for this item exceeds available stock (${resource.quantity})`
        );
      }
      ...
      if (quantity > resource.quantity) {
        throw new Error(
          `You can only add up to ${resource.quantity} items to the cart`
        );
      }
      cart.items.push({
        resource: resourceId,
